...
export default function HomePage() {
  ...
  // Prevent duplicates / double-clicks in the UI
  const [loggedSlots, setLoggedSlots] = useState({}); // { "<date>**<mealType>**<slotId>": true }
  ...
  // ✅ Ate this handler with permanent duplicate prevention via food_log.source_slot_id
  var handleAteThis = async function (date, mealType, item) {
    ...
    try {
      ...
      if (existing.items.length > 0) {
        // already logged: keep locked and stop
        return;
      }
    ...
    <div className="max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
                  ...
                  {tm.items.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-2">
                      {tm.items.map(function (item) {
                        var heroSlotKey = today + "**" + tm.meal + "**" + item.slotId;
                        var heroDisabled = !!loggedSlots[heroSlotKey];
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <button
                                ...
                                className={
                                  ...
                                  (heroDisabled ? "bg-white/10 text-white/60 cursor-not-allowed" : "bg-white/20 hover:bg-white/25")
                                }
                              >
                                {heroDisabled ? "Logged" : "✅ Ate this"}
                              </button>
                            </div>
                          </div>
                        );
                          ...
                          <th key={i} className="text-center pb-1">
                            <div
                              className={
                                ...
                                (isToday ? "bg-green-100 text-green-700 border border-green-300" : "text-gray-500")
                              }
                            >
              ...
              <div className="md:hidden">
                <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1 scrollbar-hide">
                  {weekDays.map(function (d, i) {
                    ...
                    var hasAny = MEAL_TYPES.some(function (m) {
                      return (slots[date + "**" + m] || []).length > 0;
                    });
                    ...
                    return (
                      ...
                      >
                        ...
                        {hasAny && <div className={"w-1 h-1 rounded-full mt-0.5 " + (isSelected ? "bg-white" : "bg-green-400")} />}
                      </button>
                    );
                  })}
                </div>
                        ...
                        {cellSlots.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {cellSlots.map(function (cs) {
                              var mobileSlotKey = selectedDay + "**" + meal + "**" + cs.slotId;
                              var mobileDisabled = !!loggedSlots[mobileSlotKey];
                              ...
                              return (
                                <div key={cs.slotId} className="space-y-1">
                                  ...
                                  >
                                    {mobileDisabled ? "Logged" : "✅ Ate this"}
                                  </button>
                                </div>
                              );
                            })}
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <a href="/recipes" className="text-xs text-green-600 font-medium hover:underline">
            See all
          </a>
        </div>
...
function MealCell({ date, meal, cellSlots, onAdd, onRemove, onAteThis, loggedSlots, saving }) {
  return (
    <div className="min-h-[70px] bg-white border border-gray-100 rounded-xl p-1.5 flex flex-col gap-1 group hover:border-green-200 transition-colors">
      {cellSlots.map(function (cs) {
        var slotKey = date + "**" + meal + "**" + cs.slotId;
        var disabled = !!loggedSlots[slotKey];
