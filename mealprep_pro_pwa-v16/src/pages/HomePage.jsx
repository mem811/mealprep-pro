...
// ─── WW Points estimate (classic formula: cal/50 + fat/12 - fiber/5) ───────────
function getWWPoints(recipe, mult) {
	var nut = parseNutrition(recipe?.nutrition);
	if (!nut) return null;
	var m = mult || 1;
	var cal = (nut.calories || 0) * m;
	var fat = (nut.fat || 0) * m;
	var fiber = (nut.fiber || 0) * m;
...
export default function HomePage() {
	...
	var handleAddToPlanner = async function () {
		...
		try {
			...
			if (existing.items.length > 0) {
				...
				mealPlan = await pb.collection("meal_plans").create({ user: userId, week_start_date: targetWeekStart });
			}
			await pb.collection("meal_slots").create({
				meal_plan: mealPlan.id,
	...
	var moveSlot = async function (slotId, toDate, toMeal) {
		try {
			await pb.collection("meal_slots").update(slotId, { date: toDate, slot: toMeal });
			await fetchSlots();
		} catch (e) {
			console.error("Move slot error:", e);
		}
	};
	...
	var todayMeals = MEAL_TYPES.map(function (meal) {
		return { meal, items: slots[today + "**" + meal] || [] };
	});
		...
		<div className={pageBg}>
			<div className="max-w-6xl mx-auto px-3 sm:px-6 py-6">
				<div className={shellCard + " p-4 sm:p-6"}>
															...
															<div key={item.slotId} className="flex items-start gap-2">
																...
																<div className="flex-1 min-w-0">
																	...
																	<button
																		...
																		className={"mt-1 text-[10px] font-bold px-2 py-1 rounded-lg transition-colors " + (disabled ? "bg-white/10 text-white/60 cursor-not-allowed" : "bg-white/20 hover:bg-white/25")}
																	>
					...
					{/* ── Recent Recipes strip (with Add to Planner button) ── */}
					<div className="bg-white/60 backdrop-blur rounded-[28px] border border-emerald-100/70 shadow-lg shadow-emerald-100/40 p-4 mb-6">
						...
						<div className="flex gap-3 overflow-x-auto pb-2">
							{featuredRecipes.map(function (recipe) {
								...
								var wwPts = getWWPoints(recipe, 1);
										...
										<a href={"/recipes/" + recipe.id} className="block" title={recipe.title}>
											<div className="w-full h-24 rounded-2xl overflow-hidden bg-emerald-50 mb-2 relative">
												{recipe.image_url ? (
													...
													<div className="w-full h-full flex items-center justify-center"><Utensils size={24} className="text-emerald-300" /></div>
												)}
												{macros && macros.cal > 0 && (
											...
											{wwPts !== null && (
												<p className="text-[10px] font-bold text-blue-500 mt-0.5">~{wwPts} WW pts (est.)</p>
											)}
										</a>
										<button
											onClick={function (e) {
																...
																return (
																	<td key={di} className={"align-top rounded-2xl " + (fmt(d) === today ? "bg-emerald-50/40" : "")}>
																		<MealCell
																			...
																			isLogged={isLogged}
																			saving={saving}
																			onDragStart={function (slotId, fromDate, fromMeal) {
																				setDraggedSlot({ slotId, fromDate, fromMeal });
																			}}
																			onDrop={function (toDate, toMeal) {
																				if (!draggedSlot) return;
																				if (draggedSlot.fromDate === toDate && draggedSlot.fromMeal === toMeal) {
																					setDraggedSlot(null);
																					return;
																				}
											...
											{weekDays.map(function (d, i) {
												...
												return (
													<button
														...
														className={"flex-shrink-0 flex flex-col items-center px-3 py-2 rounded-2xl border transition-all " + (isSelected ? "bg-emerald-600 text-white border-emerald-600 shadow-md" : isToday ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-white/70 text-gray-600 border-gray-100")}
													>
																	...
																	return (
																		<div key={cs.slotId} className="space-y-1">
																			...
																			<button
																				...
																				className={"w-full text-xs font-semibold py-2 rounded-2xl transition-colors " + (disabled ? "bg-gray-200/70 text-gray-500 cursor-not-allowed" : "bg-emerald-600 text-white hover:bg-emerald-700")}
																			>
																				{disabled ? "Logged" : "✅ Ate this"}
																			</button>
																		</div>
														...
														{group.items.map(function (item, i) {
															...
															return (
																<li key={i} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-emerald-50/40 transition-colors" onClick={function () { toggleCheck(checkKey); }}>
																	...
																	{item.qty > 0 && (
																		<span className={"text-xs flex-shrink-0 " + (checkedItems[checkKey] ? "text-gray-300" : "text-gray-400")}>{parseFloat(item.qty.toFixed(1))} {item.unit}</span>
																	)}
																</li>
															);
														})}
...
function MealCell({ date, meal, cellSlots, onAdd, onRemove, onAteThis, isLogged, saving, onDragStart, onDrop }) {
	...
	return (
		<div
			className={
				"min-h-[78px] backdrop-blur border rounded-3xl p-2 flex flex-col gap-2 transition-colors " +
				(isDragOver ? "bg-emerald-100/80 border-emerald-400 border-dashed" : "bg-white/70 border-emerald-100/70 hover:border-emerald-200")
			}
...
function DesktopRecipeCard({ recipe, servings, macros, onRemove, onAteThis, ateDisabled, onDragStart }) {
	...
	var wwPts = getWWPoints(recipe, servings);

	return (
		<a
			href={"/recipes/" + recipe.id}
			draggable
			onDragStart={function (e) { e.stopPropagation(); onDragStart?.(); }}
		...
		>
			{proxied ? (
				<img src={proxied} alt={recipe.title} className="w-full h-12 rounded-xl object-cover" onError={function () { setImgError(true); }} />
			) : (
			...
			{wwPts !== null && (
				<span className="px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-[9px] font-bold text-blue-600">~{wwPts} WW pts</span>
			)}
			{servings > 1 && <span className="text-[9px] text-emerald-700 font-bold">{servings}x</span>}
			<button
				disabled={ateDisabled}
				onClick={function (e) { e.preventDefault(); e.stopPropagation(); onAteThis?.(); }}
				className={"mt-1 w-full text-[9px] font-bold py-1 rounded-xl transition-colors " + (ateDisabled ? "bg-gray-200/70 text-gray-500 cursor-not-allowed" : "bg-emerald-600 text-white hover:bg-emerald-700")}
			>
...
function MobileRecipeCard({ recipe, servings, macros, onRemove }) {
	...
	return (
		...
		>
			{proxied ? (
				<img src={proxied} alt={recipe.title} className="w-9 h-9 rounded-2xl object-cover flex-shrink-0" onError={function () { setImgError(true); }} />
			) : (
			...
			<div className="flex-1 min-w-0">
				...
				{macros && (
					<p className="text-[10px] text-gray-500 font-semibold mt-0.5">🔥 {macros.cal} · P {macros.p}g · C {macros.c}g · F {macros.f}g</p>
				)}
				{wwPts !== null && (
					<p className="text-[10px] font-bold text-blue-500">~{wwPts} WW pts (est.)</p>
				)}
			</div>
			<button
				onClick={function (e) { e.preventDefault(); e.stopPropagation(); onRemove(); }}
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
			<div className="bg-white rounded-[28px] shadow-2xl p-6 w-full max-w-sm">
				...
				<div className="grid grid-cols-4 gap-2 mb-5">
					{["breakfast", "lunch", "dinner", "snack"].map(function (m) {
						...
						return (
							<button
								key={m}
								onClick={function () { onMealChange(m); }}
								className={"flex flex-col items-center py-2 rounded-2xl border-2 text-xs font-semibold transition-colors " + (selectedMeal === m ? "border-emerald-500 bg-emerald-50 text-emerald-700" : "border-gray-100 text-gray-500 hover:border-emerald-200")}
							>
				...
				<div className="flex gap-1.5 overflow-x-auto pb-1 mb-5">
					{days.map(function (d, i) {
						...
						return (
							<button
								key={dateStr}
								onClick={function () { onDayChange(dateStr); }}
								className={"flex-shrink-0 flex flex-col items-center px-2.5 py-1.5 rounded-2xl border-2 text-xs transition-colors " + (selectedDay === dateStr ? "border-emerald-500 bg-emerald-50 text-emerald-700 font-bold" : "border-gray-100 text-gray-500 hover:border-emerald-200")}
							>
				...
				<div className="flex gap-3">
					<button onClick={onClose} className="flex-1 py-2.5 rounded-2xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
						Cancel
					</button>
