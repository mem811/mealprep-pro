...
// WW Points — always per 1 serving (never multiplied by servings_multiplier)
function getWWPoints(recipe) {
	var nut = parseNutrition(recipe?.nutrition);
	if (!nut) return null;
	var cal = nut.calories || 0;
	var fat = nut.fat || 0;
	var fiber = nut.fiber || 0;
...
export default function HomePage() {
	...
	useEffect(function () {
		async function fetchLoggedForWeek() {
			...
			} catch (e) { console.error("Fetch logged-for-week error:", e); }
		}
		fetchLoggedForWeek();
	}, [weekStart, weekEnd]);
	...
	var handleRecipeSelect = async function ({ recipe, servingsMultiplier }) {
		...
		try {
			...
			await fetchSlots();
		} catch (e) { console.error("Save slot error:", e); }
	...
	var handleAddToPlanner = async function () {
		...
		} catch (e) { console.error("Add to planner error:", e); alert("Failed to add recipe to planner."); }
	...
	var moveSlot = async function (slotId, toDate, toMeal) {
		...
		catch (e) { console.error("Move slot error:", e); }
	};
	...
	var handleAteThis = async function (date, mealType, item) {
		...
		try {
			...
			await createFoodLogEntry({
				user: userId, date, meal_type: MEAL_LABELS[mealType] || mealType,
		...
		<div className={pageBg}>
			<div className="max-w-6xl mx-auto px-3 sm:px-6 py-6">
				<div className={shellCard + " p-4 sm:p-6"}>
														...
														return (
															<div key={item.slotId} className="flex items-start gap-2">
																...
																<div className="flex-1 min-w-0">
																	...
																	<button disabled={disabled} onClick={function (e) { e.preventDefault(); e.stopPropagation(); handleAteThis(today, tm.meal, item); }} className={"mt-1 text-[10px] font-bold px-2 py-1 rounded-lg transition-colors " + (disabled ? "bg-white/10 text-white/60 cursor-not-allowed" : "bg-white/20 hover:bg-white/25")}>
													...
													{weekDays.map(function (d, i) {
														...
														return (
															<th key={i} className="text-center pb-1">
																<div className={"inline-flex flex-col items-center px-3 py-1.5 rounded-2xl border " + (isToday ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-white/70 text-gray-600 border-gray-100")}>
													...
													return (
														<tr key={meal}>
															<td className="pr-2 py-1 align-top">
																<div className="text-right"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{MEAL_LABELS[meal]}</span></div>
															</td>
															{weekDays.map(function (d, di) {
																...
																return (
																	<td key={di} className={"align-top rounded-2xl " + (fmt(d) === today ? "bg-emerald-50/40" : "")}>
																		<MealCell
																			date={date} meal={meal} cellSlots={cellSlots}
																			...
																			onDrop={function (toDate, toMeal) {
																				...
																				moveSlot(draggedSlot.slotId, toDate, toMeal);
																				setDraggedSlot(null);
																			}}
																		/>
																	</td>
												...
												return (
													<div key={meal} className={"bg-gradient-to-r " + MEAL_COLORS[meal] + " border rounded-3xl p-3"}>
														<div className="flex items-center justify-between mb-2">
															...
															<button onClick={function () { openModal(selectedDay, meal); }} className="w-8 h-8 rounded-2xl bg-white/80 border border-white/60 shadow-sm flex items-center justify-center hover:bg-white transition-colors">
																...
																{cellSlots.map(function (cs) {
																	...
																	return (
																		<div key={cs.slotId} className="space-y-1">
																			...
																			<button disabled={disabled} onClick={function (e) { e.preventDefault(); e.stopPropagation(); handleAteThis(selectedDay, meal, cs); }} className={"w-full text-xs font-semibold py-2 rounded-2xl transition-colors " + (disabled ? "bg-gray-200/70 text-gray-500 cursor-not-allowed" : "bg-emerald-600 text-white hover:bg-emerald-700")}>
																				{disabled ? "Logged" : "✅ Ate this"}
																			</button>
																		</div>
																	);
											...
											return (
												<div key={gi}>
													<div className="px-4 py-2 bg-emerald-50/40 border-b border-emerald-100/60">
														<span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{group.icon} {group.category}</span>
													</div>
													<ul className="divide-y divide-emerald-100/60">
														{group.items.map(function (item, i) {
															var checkKey = item.name.toLowerCase().trim();
															return (
																<li key={i} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-emerald-50/40 transition-colors" onClick={function () { toggleCheck(checkKey); }}>
					...
					{/* ── Recent Recipes strip — BELOW the planner ── */}
							...
							{featuredRecipes.map(function (recipe) {
								...
								return (
									<div key={recipe.id} className="flex-shrink-0 w-40 bg-white/70 rounded-3xl border border-emerald-100/60 shadow-sm p-3 hover:shadow-md transition-shadow group">
										...
										<button
											onClick={function (e) { e.preventDefault(); setAddToPlannerRecipe({ id: recipe.id, title: recipe.title }); setAddToPlannerDay(today); setAddToPlannerMeal("lunch"); }}
											className="mt-2 w-full text-[10px] font-bold py-1.5 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700 transition-colors"
										>
											+ Add to Planner
										</button>
									</div>
					...
					<AddToPlannerModal
						recipe={addToPlannerRecipe} selectedDay={addToPlannerDay} selectedMeal={addToPlannerMeal}
						onDayChange={setAddToPlannerDay} onMealChange={setAddToPlannerMeal}
						onConfirm={handleAddToPlanner} onClose={function () { setAddToPlannerRecipe(null); }} saving={saving}
					/>
...
function MealCell({ date, meal, cellSlots, onAdd, onRemove, onAteThis, isLogged, saving, onDragStart, onDrop }) {
	...
	return (
		<div
			className={"min-h-[78px] backdrop-blur border rounded-3xl p-2 flex flex-col gap-2 transition-colors " + (isDragOver ? "bg-emerald-100/80 border-emerald-400 border-dashed" : "bg-white/70 border-emerald-100/70 hover:border-emerald-200")}
		...
		>
			{cellSlots.map(function (cs) {
				return (
					<DesktopRecipeCard
						key={cs.slotId} recipe={cs.recipe} servings={cs.servings_multiplier}
						...
						onDragStart={function () { onDragStart(cs.slotId, date, meal); }}
					/>
				);
			})}
			<button onClick={onAdd} disabled={saving} className="flex items-center justify-center w-full mt-auto py-2 rounded-2xl border border-dashed border-emerald-100/80 hover:border-emerald-300 hover:bg-emerald-50/60 transition-colors">
...
function DesktopRecipeCard({ recipe, servings, macros, onRemove, onAteThis, ateDisabled, onDragStart }) {
	...
	// WW points always per 1 serving — pass no multiplier
	var wwPts = getWWPoints(recipe);

	return (
		<a href={"/recipes/" + recipe.id} draggable onDragStart={function (e) { e.stopPropagation(); onDragStart?.(); }}
			className="flex flex-col items-center gap-1 bg-white/70 rounded-2xl border border-emerald-100/60 p-2 group/card relative hover:bg-emerald-50/50 transition-colors cursor-grab active:cursor-grabbing"
			title={recipe.title}
		>
			...
			<button disabled={ateDisabled} onClick={function (e) { e.preventDefault(); e.stopPropagation(); onAteThis?.(); }} className={"mt-1 w-full text-[9px] font-bold py-1 rounded-xl transition-colors " + (ateDisabled ? "bg-gray-200/70 text-gray-500 cursor-not-allowed" : "bg-emerald-600 text-white hover:bg-emerald-700")}>
				{ateDisabled ? "Logged" : "✅ Ate this"}
			</button>
			<button onClick={function (e) { e.preventDefault(); e.stopPropagation(); onRemove(); }} className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-white border border-emerald-100 text-gray-500 hidden group-hover/card:flex items-center justify-center shadow-sm">
				<X size={12} />
			</button>
		</a>
	);
...
function MobileRecipeCard({ recipe, servings, macros, onRemove }) {
	...
	if (!recipe) return null;
	var proxied = !imgError && recipe.image_url ? getProxiedImage(recipe.image_url) : null;
	// WW points always per 1 serving
	var wwPts = getWWPoints(recipe);
	...
	return (
		<a href={"/recipes/" + recipe.id} className="flex items-center gap-3 bg-white/70 backdrop-blur rounded-3xl px-3 py-3 border border-emerald-100/60 shadow-sm hover:bg-emerald-50/40 transition-colors">
			...
			<button onClick={function (e) { e.preventDefault(); e.stopPropagation(); onRemove(); }} className="w-8 h-8 rounded-2xl hover:bg-red-50 flex items-center justify-center transition-colors flex-shrink-0">
...
function AddToPlannerModal({ recipe, selectedDay, selectedMeal, onDayChange, onMealChange, onConfirm, onClose, saving }) {
	...
	var DAY_NAMES_MOD = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
	return (
			...
			<div className="bg-white rounded-[28px] shadow-2xl p-6 w-full max-w-sm">
				...
				<div className="flex gap-1.5 overflow-x-auto pb-1 mb-5">
					{days.map(function (d, i) {
						...
						var dayName = i === 0 ? "Today" : DAY_NAMES_MOD[(d.getDay() + 6) % 7];
						return (
