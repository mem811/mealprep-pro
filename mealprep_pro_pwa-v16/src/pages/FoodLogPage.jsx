...
export default function FoodLogPage() {
	...
	function buildAddFoodFromOff(barcode, product) {
		...
		return { barcode, name, meal_type: "Snack", grams: 100, per100g, notes: "" };
	}
	...
	return (
		<div className="p-4 max-w-3xl mx-auto">
			...
			<div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
				{[["Calories", Math.round(totals.calories), ""], ["Protein", Math.round(totals.protein), "g"], ["Carbs", Math.round(totals.carbs), "g"], ["Fat", Math.round(totals.fat), "g"]].map(([label, val, unit]) => (
								...
								<div className="flex items-start justify-between gap-3">
									<div className="min-w-0">
										<div className="flex items-center gap-2 flex-wrap">
											<span className={"inline-flex items-center px-2 py-1 rounded-full border text-xs font-bold " + mealPillClass(e.meal_type)}>
												{e.meal_type || "Meal"}
											</span>
									...
									<div className="flex flex-col items-end gap-2 flex-shrink-0">
										<div className="text-xs text-gray-500">
											{e.created ? new Date(e.created).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : ""}
										</div>
										<div className="flex items-center gap-2">
											<button onClick={() => requestEdit(e)} className="text-xs font-semibold text-gray-700 hover:text-gray-900">Edit</button>
											<button onClick={() => requestDelete(e)} disabled={deletingId === e.id} className="text-xs font-semibold text-red-600 hover:text-red-700 disabled:opacity-50">
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<div className="relative w-[92%] max-w-md rounded-2xl bg-white shadow-xl border border-gray-100 p-4">
						...
						{scanError ? (
							...
							<div className="mt-3 text-xs text-gray-500">Point your camera at the barcode on the label.</div>
						)}
						{/* Manual fallback inside modal */}
						<div className="mt-3 flex gap-2">
							...
							/>
							<button
								onClick={async () => { setScanOpen(false); await doLookup(barcodeInput); }}
								disabled={lookingUp}
			...
			{confirmDelete && (
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<div className="relative w-[92%] max-w-sm rounded-2xl bg-white shadow-xl border border-gray-100 p-4">
						...
						<div className="mt-4 flex items-center justify-end gap-2">
							<button className="px-3 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-semibold hover:bg-gray-200 disabled:opacity-50" onClick={() => setConfirmDelete(null)} disabled={!!deletingId}>Cancel</button>
							<button className="px-3 py-2 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 disabled:opacity-50" onClick={confirmDeleteNow} disabled={!!deletingId}>{deletingId ? "Deleting..." : "Delete"}</button>
						</div>
					</div>
				</div>
			)}
			...
			{editing && (
				<div className="fixed inset-0 z-50 flex items-center justify-center">
					...
					<div className="relative w-[92%] max-w-lg rounded-2xl bg-white shadow-xl border border-gray-100 p-4">
						...
						<div className="mt-4 flex items-center justify-end gap-2">
							<button className="px-3 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-semibold hover:bg-gray-200 disabled:opacity-50" onClick={() => setEditing(null)} disabled={savingEdit}>Cancel</button>
							<button className="px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50" onClick={saveEdit} disabled={savingEdit}>{savingEdit ? "Saving..." : "Save"}</button>
						</div>
					</div>
				</div>
			)}
